...
test("PWA 01 · Every shipped script and manifest parses; HTML dependencies exist at repo root",()=>{
  ...
  const html=fs.readFileSync(path.join(project,"index.html"),"utf8");assert(html.includes("<small>v3.1</small>"));
  ...
  const scriptOrder=[...html.matchAll(/<script defer src="([^"]+)"/g)].map(match=>match[1]);assert.deepEqual(scriptOrder,["./storage.js?v=3.1","./app.js?v=3.1","./share.js?v=3.1","./pwa.js?v=3.1"]);
});
test("PWA 02 · Manifest has a distinct identity, valid install icons and in-scope launch paths",()=>{
  ...
  for(const base of ["https://example.test/Multipuls/","https://example.test/renamed/nested/","https://example.test/"]){
    const start=new URL(manifest.start_url,base+"manifest.webmanifest"),scope=new URL(manifest.scope,base+"manifest.webmanifest");assert(start.href.startsWith(scope.href));assert.equal(start.search,"?v=3.1");
  }
...
test("PWA 03 · First installation precaches all required files with fresh requests",async()=>{
  ...
  assert((await w.cached(w.constants.ENTRY)).includes("<small>v3.1</small>"));
});
test("PWA 04 · HTML navigation is network-first and saves a successful new response",async()=>{
  ...
  const response=await w.navigate(w.constants.ROOT+"?v=3.1");assert.equal(await response.text(),"new");assert.equal(w.requests[0].options.cache,"no-store");assert.equal(await w.cached(w.constants.ENTRY),"new");
});
test("PWA 05 · Offline root and versioned launches use the cached HTML and static assets",async()=>{
  ...
  for(const url of [w.constants.ROOT,w.constants.ROOT+"?v=3",w.constants.ROOT+"?v=3.1",w.constants.ENTRY]){const response=await w.navigate(url);assert.equal(response.status,200);assert((await response.text()).includes("Multipuls"));}
...
test("PWA 07 · Activation deletes only older Multipuls caches within this app path",async()=>{
  const w=worker();const old=w.constants.PREFIX+"v3",other="multipuls:/Other/:v2";
...
test("PWA 09 · Versioned script URLs have distinct cache entries during updates",async()=>{
  const w=worker();await w.dispatch("install");const previous=new URL("app.js?v=3.1",w.constants.ROOT).href,next=new URL("app.js?v=4",w.constants.ROOT).href;
...
test("PWA 10 · Failed installation does not activate or delete the older cache",async()=>{
  const w=worker(),old=w.constants.PREFIX+"v3";w.entries.set(old,new Map());w.network=async()=>new Response("missing",{status:404});
...
test("PWA 12 · Cache and precache URLs follow the registered project subdirectory",async()=>{
  const w=worker("https://example.test/another/project/");assert(w.constants.ASSETS.every(url=>url.startsWith("https://example.test/another/project/")));assert.equal(w.constants.CACHE,"multipuls:/another/project/:v3.1");await w.dispatch("install");assert.equal(w.skipped,1);
});
...
(async()=>{
  ...
  fs.writeFileSync(path.join(__dirname,"pwa-results.json"),JSON.stringify({version:"v3.1",method:"Static release checks and deterministic service-worker event tests with real Response/Request objects and in-memory Cache/Fetch doubles. No browser or network execution.",tests:reports},null,2));
